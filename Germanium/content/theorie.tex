...
\begin{figure}[H]
  ...
  \label{fig:Extin}
\end{figure}
Wie in \ref{fig:Extin} zu sehen ist dominieren bei verschiedenen Energien
...
\begin{equation}
  \sigma_{\text{Photo}}\sim Z^5 E_{\gamma}^{-7/2}.
  \label{eqn:WQphoto}
\end{equation}
\cite{Karlsruhe}
\\
...
\begin{equation}
  E_{max}=E_{\gamma}\frac{2\epsilon}{1+2\epsilon}\;\;<E_{\gamma}.
  \label{eqn:kante}
\end{equation}
[Springer3]
...
\end{equation}
mit $\epsilon=\sfrac{E_{\gamma}}{m_e c^2}$ und dem Thomsonschen Schreuquerschnitt $\sigma_{\text{Th}}=\sfrac{8\pi}{3}r_e^2$.
...
\begin{equation}
  \sigma_{\text{Paar}}\sim Z^2\ln(\frac{2E_{\gamma}}{m_e c^2})
  \label{eqn:WQPaar}
\end{equation}

\begin{figure}[H]
  \centering
  \includegraphics[height=7cm]{Effekte.png}
...
Halbleiter werden zwischen direkten und indirekten Halbleitern unterschieden. Bei direkten Halbleitern
...
Werden p- und n-dotierte Halbleiter zusammengebracht werden entsteht ein pn-Übergang, in einem
Teilbereich vernichten sich Elektronen und Löcher wodurch sich eine Verarmungszone bildet.
...
Bei dem Verwendeten Detektor handelt es sich um einen koaxialen Ge-Detektor wie in
...
Verarmungszone im Detektor. Um diese Verarmungszone, den Detektorbereich, zu erreichen müssen die
Photonen erst die Al-Schicht und die Li-Schicht durchdringen, wodurch es zu einer unteren Nachweisgrenze
für Gammaenergien kommt. Für den hier verwendeten Detektor liegt die untere Nachweisgrenze
bei 40 bis 50\;keV.
...
Dringt ein ionisierendes Teilchen in die Verarmungszone ein erzeugt es ein Elektronen-Loch-Paar, duch die Spannung
wird das Paar räumlich getrennt wodurch eine Rekombination verhindert wird. Der dadurch entstehende Ladungsimpuls
...
Um die Vollenergienachweiswahrscheinlichkeit bestimmen zu können muss die aktuelle Aktivität der
Probe bekannt sein. Dieser theoretisch Werte für alle Energien kann mit den Messergebnissen,
die über alle Energien summiert werden, verglichen werden um die Vollenergienachweiswahrscheinlichkeit
zu erhalten.

\section{Das Spektrum eines monochromatischen Gammastrahlers}
Das Spektrum eines monochromatischen Gammastrahlers zeigt mehrere Besonderheiten auf, wie in \ref{fig:Spektrum} zu
